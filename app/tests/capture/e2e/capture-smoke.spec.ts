...
test("dispose failures replace success with a failing artifact", async () => {
  const { directory, resultPath, electronApplication } =
    await launchCaptureHarness({ injectDisposeFailure: true });
  let applicationClosed = false;
  try {
    const result = await waitForResult(resultPath);
...
test("readiness terminal reporter persists artifact-write failures", async () => {
  const { directory, resultPath, electronApplication } =
    await launchCaptureHarness({ injectArtifactFailure: true });
  try {
    const result = await waitForResult(resultPath);
    expect(result.ok).toBe(false);
    expect(result.error).toContain("Capture readiness chain failed");
    expect(result.error).toContain("injected capture artifact write failure");
  } finally {
    try {
      await electronApplication.close();
    } finally {
...
test("shutdown terminal reporter replaces a stale success artifact", async () => {
  const { directory, resultPath, electronApplication } =
    await launchCaptureHarness({ injectShutdownChainFailure: true });
  let applicationClosed = false;
  try {
    const initialResult = await waitForResult(resultPath);
    expect(initialResult.ok, initialResult.error).toBe(true);

    await electronApplication.close();
    applicationClosed = true;
    const shutdownResult = await readResult(resultPath);
    expect(shutdownResult.ok).toBe(false);
    expect(shutdownResult.error).toContain("Capture shutdown chain failed");
    expect(shutdownResult.error).toContain(
      "injected capture shutdown chain failure",
    );
  } finally {
    try {
      if (!applicationClosed) {
...
interface CaptureHarnessOptions {
  injectDisposeFailure?: boolean;
  injectArtifactFailure?: boolean;
  injectShutdownChainFailure?: boolean;
}

async function launchCaptureHarness(options: CaptureHarnessOptions = {}) {
  ...
  const electronApplication = await electron.launch({
    ...
    env: {
      ...
      ...(options.injectDisposeFailure
        ? { CAPTURE_SMOKE_DISPOSE_FAILURE: "1" }
        : {}),
      ...(options.injectArtifactFailure
        ? { CAPTURE_SMOKE_ARTIFACT_FAILURES: "2" }
        : {}),
      ...(options.injectShutdownChainFailure
        ? { CAPTURE_SMOKE_SHUTDOWN_CHAIN_FAILURE: "1" }
        : {}),
    },
  });
